/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改腾讯云凭证配置
 */
...
export async function handleSettingsSave(
  ...
  // 读取现有配置，仅更新用户填写了的字段
  const existingConfig = store.getDecryptedConfig(installationId);
  const newConfig: Record<string, string> = { ...existingConfig };

  if (secretId) newConfig.tencent_secret_id = secretId;
  if (secretKey) newConfig.tencent_secret_key = secretKey;
  // 区域允许清空（使用默认值），所以特殊处理
  newConfig.tencent_region = region;

  // 加密保存
  store.saveEncryptedConfig(installationId, newConfig);
...
function renderConfigForm(
  ...
  const cfg = store.getDecryptedConfig(installationId);
  const maskedSecretId = maskValue(cfg.tencent_secret_id || "");
  const maskedSecretKey = maskValue(cfg.tencent_secret_key || "");
  const currentRegion = cfg.tencent_region || "ap-guangzhou";
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>腾讯云 — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>腾讯云设置</h1>
    <p class="desc">修改您的腾讯云 API 密钥配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 SecretId</dt>
      <dd>${maskedSecretId || "（未配置）"}</dd>
      <dt>当前 SecretKey</dt>
      <dd>${maskedSecretKey || "（未配置）"}</dd>
      <dt>当前区域</dt>
      <dd>${escapeHtml(currentRegion)}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="tencent_secret_id" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://console.cloud.tencent.com/cam/capi" target="_blank">腾讯云 API 密钥管理</a> 查看</p>

      <label>SecretKey</label>
      <input name="tencent_secret_key" type="password" placeholder="留空则不修改" />

      <label>区域</label>
      <input name="tencent_region" placeholder="留空则使用默认值 ap-guangzhou" value="${escapeHtml(cfg.tencent_region || "")}" />
      <p class="hint">默认 ap-guangzhou</p>
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-tencent">自行部署</a>本 App</li>
        </ul>
      </div>
